...
def rabbitmq_container() -> Generator[RabbitMqContainer, None, None]:
    with (
        ...
        .waiting_for(LogMessageWaitStrategy("Server startup complete")) as rabbitmq_container
    ):
...
async def rabbitmq_vhost(rabbitmq_host: str, rabbitmq_management_port: int) -> AsyncGenerator[str, None]:
...
@pytest.fixture(scope="function")
async def settings(rabbitmq_host: str, rabbitmq_port: int, rabbitmq_vhost: str) -> KhnmSettings:
    return KhnmSettings(
...
@pytest.fixture(scope="function")
async def amqp_connection(settings: KhnmSettings) -> AsyncGenerator[AbstractRobustConnection, None]:
